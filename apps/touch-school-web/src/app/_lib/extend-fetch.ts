...
export type ResponseGenericBody<RES> = Omit<Awaited<ReturnType<typeof fetch>>, keyof Body | 'clone'> & {
  body: RES;
};

export type JsonResponse<RES> = RES extends object ? ResponseGenericBody<RES> : ResponseGenericBody<string>;
...
const extendFetch =
  ...
  async <RES>(...args: FetchArgs): Promise<JsonResponse<RES>> => {
    ...
    if (defaultOptions?.interceptors?.request) {
      requestInterceptorAppliedArgs = await defaultOptions.interceptors.request(defaultOptionAppliedArgs);
    } else {
    ...
    try {
      const response = await fetchProvided(fetchUrl, {
        ...
        body:
          ...
          (isJsonRequest ? JSON.stringify(fetchRequestInit.body) : (fetchRequestInit.body as BodyInit)),
      });
    ...
    const interceptedResponse =
      (await defaultOptions?.interceptors?.response?.(jsonResponse, requestInterceptorAppliedArgs)) || jsonResponse;
